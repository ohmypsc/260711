...
const useAttendanceIds = () => {
  ...
  const removeId = (id: number) => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(getIds().filter((x) => x !== id))
    );
  };
    ...
    <div className="attendance-wrapper">
              ...
              <div key={row.id} className="attendance-card">
                ...
                <div className="actions">
                  ...
                  <button
                    ...
                    onClick={() => setOpenModal({ type: "delete", row })}
                  >
      ...
      {typeof openModal === "object" && openModal?.type === "edit" && (
        <EditAttendanceModal
          ...
          onClose={() => setOpenModal(null)}
          onSuccess={(updated) =>
            setMyRows((prev) => prev.map((r) => (r.id === updated.id ? updated : r)))
          }
          onToast={handleToast}
        />
      )}
...
function WriteAttendanceModal({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        .from("attendance")
        .insert([{ ...form, phone: normalizePhone(form.phone) }])
        .select("*")
        .single();
    ...
    <Modal
      ...
      footer={
        <div className="attendance-footer-row">
          <Button variant="submit" type="submit" form="write-form" disabled={loading}>
            전달하기
          </Button>
        ...
        <form id="write-form" className="attendance-form" onSubmit={onSubmit}>
          <div className="field span-2">
            ...
            <div className="toggle-row">
              <button
                type="button"
                className={`toggle-btn ${form.side === "groom" ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`toggle-btn ${form.side === "bride" ? "active" : ""}`}
          ...
          <div className="field span-2">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  phone: formatPhone(normalizePhone(e.target.value)),
                })
              }
            />
          </div>
          ...
          <div className="field span-2">
            ...
            <div className="toggle-row">
              <button
                type="button"
                className={`toggle-btn ${form.meal === "yes" ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`toggle-btn ${form.meal === "no" ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`toggle-btn ${form.meal === "unknown" ? "active" : ""}`}
                onClick={() => setForm({ ...form, meal: "unknown" })}
              >
                미정
              </button>
            </div>
...
function FindAttendanceModal({
  ...
  const onFind = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("name", name.trim())
        .or(`phone.eq.${phone},phone.eq.${normPhone}`)
        .maybeSingle();
  ...
  return (
    <Modal
      ...
      footer={
        <div className="attendance-footer-row">
          <Button variant="submit" type="submit" form="find-form" disabled={loading}>
            찾기
          </Button>
          <Button variant="close" onClick={onClose}>
            닫기
        ...
        <form id="find-form" className="attendance-form" onSubmit={onFind}>
          ...
          <div className="field span-2">
            ...
            <input
              ...
              inputMode="numeric"
              value={phone}
              onChange={(e) => setPhone(formatPhone(normalizePhone(e.target.value)))}
            />
          </div>
        </form>
      </div>
...
function EditAttendanceModal({
  ...
  const onUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        .from("attendance")
        .update({ ...form, phone: normalizePhone(form.phone) })
        .eq("id", row.id)
        .select("*")
        .single();
  ...
  return (
    <Modal
      ...
      footer={
        <div className="attendance-footer-row">
          <Button variant="submit" type="submit" form="edit-form" disabled={loading}>
            수정하기
          </Button>
          <Button variant="close" onClick={onClose}>
            닫기
...
function DeleteAttendanceModal({
  ...
  return (
    <Modal
      ...
      footer={
        <div className="attendance-footer-row">
          <Button variant="submit" onClick={onDelete} disabled={loading}>
            삭제하기
          </Button>
          <Button variant="close" onClick={onClose}>
            취소
    ...
    >
      <div className="attendance-modal-content">
        ...
        <p
          className="modal-subtitle keep-all"
          style={{
            textAlign: "center",
            margin: "20px 0",
            color: "var(--text-main)",
