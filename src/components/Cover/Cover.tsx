...
/** Leaf line divider (SVG Path 유지) */
function LeafLine({ variant }: { variant: "top" | "bottom" }) {
  // ... (SVG path 코드는 기존과 동일하므로 생략 가능, 컬러는 SCSS에서 currentColor로 제어됨) ...
  const EndCaps =
    variant === "top" ? (
      <>
        <path
      ...
      <div className="family-section">
        ...
        <div className="name">
          <span className="parent-names">
            {names.groomFather} · {names.groomMother}
          </span>
          <span className="relation-name--adjust">의 아들</span>
          <span className="groom-bride-name">{names.groom}</span>
        </div>

        <div className="name">
          <span className="parent-names">
            {names.brideFather} · {names.brideMother}
          </span>
          <span className="relation-name--adjust">의 딸</span>
          <span className="groom-bride-name">{names.bride}</span>
        </div>
